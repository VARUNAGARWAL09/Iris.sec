...
export function SimulationProvider({ children }: { children: ReactNode }) {
  ...
  // Update response time only when a new alert arrives — random value between 14m 00s and 14m 59s
  const MIN_RESPONSE_SECONDS = 14 * 60;        // 840s (14m 00s)
  const MAX_RESPONSE_SECONDS = 14 * 60 + 59;   // 899s (14m 59s)

  useEffect(() => {
    // Fires whenever a new alert is added; no timer, no continuous ticking
    const randomSeconds = Math.floor(
      MIN_RESPONSE_SECONDS + Math.random() * (MAX_RESPONSE_SECONDS - MIN_RESPONSE_SECONDS + 1)
    );
    setLiveAvgResponseTime(formatResponseTime(randomSeconds));
  }, [alerts.length]);
  ...
  const generateAlert = useCallback(async () => {
    ...
    try {
        ...
        enrichedData = await enrichAlertData(enrichedRawData, scenario.title, scenario.description);
    } catch (e) {
    ...
    if (alertData) {
      ...
      try {
          ...
          await executeAutomatedResponse(alertData.id, severity, enrichedData, enrichedRawData);
      } catch (e) {
