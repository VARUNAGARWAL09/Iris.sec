...
export function AlertCard({ alert, index = 0, showDetails = false }: AlertCardProps) {
  ...
  const { classifyThreat, loading } = useMLInference();
  const [mlClassification, setMlClassification] = useState<any>(null);

  const formatTimeAgo = (date: Date | string) => {
    const dateObj = typeof date === 'string' ? new Date(date) : date;
    const timeDiff = virtualTime.getTime() - dateObj.getTime();
    ...
    // Otherwise use formatDistanceToNow for older dates
          ...
          {analysis && (
            <button
              onClick={async (e) => { 
                ...
                if (newExpanded && !mlClassification) {
                  const result = await classifyThreat(alert.title, alert.description || '');
              ...
              {mlClassification && (
                <div className="p-2 rounded bg-primary/5 border border-primary/10">
                  <div className="flex items-center gap-2 text-xs font-semibold text-primary mb-1">
                    <Shield className="h-3 w-3" />
                    ML CLASSIFICATION: {mlClassification.threat_category.toUpperCase()}
                  </div>
                  <p className="text-[10px] text-muted-foreground leading-tight italic">
                    {mlClassification.explanation}
                  </p>
                </div>
              )}
