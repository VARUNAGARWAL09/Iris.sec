...
export function AssistantChatbot() {
    ...
    const { recognizeIntent } = useMLInference();
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        try {
            // Real ML intent recognition
            const mlIntent = await recognizeIntent(userMsg.content);
            
            // Simulate AI processing delay
            setTimeout(() => {
                const response = generateResponse(userMsg.content, mlIntent);
    ...
    const generateResponse = (query: string, mlIntent?: any): Message => {
        const lowerQuery = query.toLowerCase();
        const timestamp = new Date();

        // Integrate ML Intent if available
        let mlSuffix = "";
        if (mlIntent) {
            mlSuffix = `\n\n--- 🧠 **IRIS ML INSIGHT** ---\n` +
                       `• **Detected Intent:** ${mlIntent.detected_intent.replace('_', ' ')}\n` +
                       `• **Confidence:** ${(mlIntent.confidence * 100).toFixed(1)}%\n` +
                       (mlIntent.extracted_entities.length > 0 ? `• **Entities:** ${mlIntent.extracted_entities.map((e: any) => e.value).join(', ')}` : "");
        }
