...
export function Header() {
  ...
  return (
    <header
      className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/50 px-6"
      style={{
        background: 'hsl(222 40% 6% / 0.85)',
        backdropFilter: 'blur(16px)',
        WebkitBackdropFilter: 'blur(16px)',
        boxShadow: '0 1px 0 hsl(152 100% 48% / 0.08), inset 0 1px 0 hsl(210 30% 94% / 0.03)',
      }}
    >
      {/* Left section - Search */}
      ...
      <div className="flex items-center gap-2.5">
        {/* Live Threat Level chip */}
        <div className="hidden lg:flex items-center gap-1.5 rounded-md border border-severity-high/20 bg-severity-high/8 px-2.5 py-1">
          <span className="h-1.5 w-1.5 rounded-full bg-severity-high animate-pulse" />
          <span className="font-mono text-[10px] font-semibold uppercase tracking-widest text-severity-high">Threat: HIGH</span>
        </div>

        {/* Real-time ZULU Clock */}
        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-md border border-border/50" style={{ background: 'hsl(220 30% 10% / 0.6)' }}>
          <Clock className="h-3.5 w-3.5 text-primary/70" />
          <span className="font-mono text-sm font-medium text-foreground/90">
            {time.toISOString().replace('T', ' ').substring(0, 19)}
          </span>
          <span className="font-mono text-[10px] font-bold text-primary/60 tracking-wider">Z</span>
        </div>
