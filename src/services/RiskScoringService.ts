...
export async function calculateRiskScore(alertId: string): Promise<RiskScoringResult | null> {
    try {
        ...
        const ruleAdjustedScore = Math.min(100, Math.max(0, baseScore + totalAdjustment));

        const adjustedScore = ruleAdjustedScore;

        // Determine if escalation needed based on rule-adjusted score
        ...
        if (adjustments.length > 0) {
            updateAlertRiskMetadata(alertId, {
                ...
                rule_adjusted_score: ruleAdjustedScore,
