 ...
 */
export async function parseLogFile(content: string, fileName: string): Promise<ParsedLogResult> {
    ...
    // Process each detection rule concurrently.
    const detectionsPromises = DETECTION_RULES.map(async (rule) => {
        ...
        if (matches.length > 0) {
            // Calculate dynamic risk score (rule-based)
            ...
            const finalScore = dynamicRiskScore;
            const finalSeverity = determineSeverity(finalScore);
    ...
    const resolvedDetections = await Promise.all(detectionsPromises);
    resolvedDetections.forEach(detection => {
        if (detection) {
            detectionsMap.set(detection.rule.id, detection);
        }
    });
