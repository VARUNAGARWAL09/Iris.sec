  ...
  return (
    <MainLayout>
      <div className="space-y-4 md:space-y-6">
        ...
        {/* Main Content Grid — single unified layout */}
        <div className="grid gap-4 md:gap-6 lg:grid-cols-3">

          {/* LEFT: Active Incidents + Activity Feed stacked */}
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* MITRE ATT&CK Tactics Monitor */}
            {(() => {
              const tacticMap: { tactic: string; keywords: string[]; color: string }[] = [
                { tactic: 'Initial Access',       keywords: ['phishing', 'login', 'bruteforce', 'brute force'],          color: '#f97316' },
                { tactic: 'Execution',            keywords: ['malware', 'ransomware', 'cryptominer'],                     color: '#ef4444' },
                { tactic: 'Persistence',          keywords: ['malware', 'ransomware', 'unauthorized'],                    color: '#ec4899' },
                { tactic: 'Privilege Escalation', keywords: ['unauthorized', 'privilege', 'escalat'],                     color: '#a855f7' },
                { tactic: 'Defense Evasion',      keywords: ['malware', 'cryptominer', 'ransomware'],                     color: '#8b5cf6' },
                { tactic: 'Credential Access',    keywords: ['login', 'bruteforce', 'brute force', 'phishing'],           color: '#6366f1' },
                { tactic: 'Discovery',            keywords: ['scan', 'recon', 'bruteforce'],                              color: '#3b82f6' },
                { tactic: 'Lateral Movement',     keywords: ['unauthorized', 'login', 'bruteforce'],                      color: '#06b6d4' },
                { tactic: 'Collection',           keywords: ['exfiltration', 'data'],                                     color: '#14b8a6' },
                { tactic: 'Exfiltration',         keywords: ['exfiltration', 'data exfil'],                               color: '#10b981' },
                { tactic: 'Command & Control',    keywords: ['cryptominer', 'malware', 'ransomware'],                     color: '#84cc16' },
                { tactic: 'Impact',               keywords: ['ransomware', 'cryptominer', 'data exfil', 'exfiltration'],  color: '#eab308' },
              ];

              const alertTitles = alerts.map(a => a.title.toLowerCase());
              const maxCount = alerts.length || 1;

              const tactics = tacticMap.map(({ tactic, keywords, color }) => {
                const hits = alertTitles.filter(t => keywords.some(k => t.includes(k))).length;
                return { tactic, hits, color, pct: Math.min(100, Math.round((hits / maxCount) * 100)) };
                ...
                >
                  <div className="flex items-center justify-between border-b border-border px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-mono text-sm font-semibold uppercase tracking-wider">MITRE ATT&CK Tactics</h3>
                      <span className="rounded-full bg-primary/20 px-2 py-0.5 text-xs font-semibold text-primary">
                  ...
                  <div className="p-4 space-y-2.5">
                    {tactics.length === 0 ? (
                      <p className="text-center text-xs text-muted-foreground py-8">No tactics detected yet — simulation starting…</p>
                    ) : (
                      tactics.map(({ tactic, hits, color, pct }, idx) => (
                        <motion.div
                          key={tactic}
                        ...
                        >
                          <span className="text-xs text-muted-foreground w-36 flex-shrink-0 truncate">{tactic}</span>
                          ...
                          <span className="text-xs font-mono font-semibold w-8 text-right flex-shrink-0" style={{ color }}>
                            {hits}
                          </span>
                        </motion.div>
                      ))
            ...
            >
              <div className="flex items-center justify-between border-b border-border px-5 py-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-mono text-sm font-semibold uppercase tracking-wider">Evidence Intelligence</h3>
                  <span className="rounded-full bg-primary/20 px-2 py-0.5 text-xs font-semibold text-primary">
                    {evidence.length}
                  </span>
                </div>
                <Button variant="ghost" size="sm" className="gap-1 text-xs" onClick={() => navigate('/evidence')}>
              ...
              <div className="p-4 space-y-2">
                {evidence.slice(0, 6).map((ev, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-base w-6 text-center flex-shrink-0">{typeIcon[ev.type] || '?'}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-mono truncate text-muted-foreground">{ev.value}</p>
                        <p className="text-xs text-muted-foreground/60 capitalize">{ev.type}</p>
                      </div>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-semibold capitalize ${classColor[ev.classification] || 'text-slate-400 bg-slate-500/10'}`}>
                        {ev.classification}
                      </span>
                    </motion.div>
                  );
                ...
                {evidence.length === 0 && (
                  <p className="text-center text-xs text-muted-foreground py-6">No evidence collected yet</p>
                )}
              </div>
            </motion.div>
          </div>

          {/* RIGHT: All sidebar widgets stacked */}
          ...
          {[
            {
              ...
              value: `${Math.round((alerts.filter(a => a.severity === 'critical').length / Math.max(alerts.length, 1)) * 100)}%`,
              sub: 'of all alerts',
              color: 'text-red-400',
              bg: 'bg-red-500/10',
            },
            {
              ...
              sub: `${evidence.filter(e => e.classification === 'malicious').length} malicious`,
            ...
            },
            {
              ...
              value: (() => {
                ...
                return alerts.filter(a => a.created_at > oneHourAgo).length;
              })(),
              sub: 'last 60 minutes',
              color: 'text-blue-400',
              bg: 'bg-blue-500/10',
            ...
            >
              <div className={`rounded-lg p-2.5 flex-shrink-0 ${bg}`}>
