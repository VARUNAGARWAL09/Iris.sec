...
const LogIngestion = () => {
    ...
    const handleParseLogs = async () => {
        ...
        try {
            ...
            // Parse log file
            const result = await parseLogFile(content, selectedFile.name);
    ...
    const handleGenerateAlerts = async () => {
        if (!parseResult || !selectedFile) {
            toast({
                ...
                description: 'Please analyze the log file first by clicking "Analyze Logs"',
                variant: 'destructive',
            });
            return;
        }

        if (parseResult.detections.length === 0) {
            toast({
                title: 'No Threats Detected',
                description: 'The log file analysis found no security threats to generate alerts for',
                variant: 'destructive',
            });
            return;
        }
        ...
        console.log('📊 Parse result:', {
            ...
            detectionsCount: parseResult.detections.length,
            detections: parseResult.detections.map(d => ({
                rule: d.rule.name,
                severity: d.severity,
        ...
        try {
            ...
            toast({
                ...
                description: error instanceof Error ? error.message : 'An error occurred while creating alerts',
                variant: 'destructive',
            });
        } finally {
            setTimeout(() => {
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <span>Step 1: Analyze log file for threats</span>
                            </div>
                            <Button
                        ...
                        {parseResult && !isProcessing && (
                            <div className="space-y-2">
                                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                    ...
                                    <span>Step 2: Generate alerts from detected threats</span>
                                </div>
                                <Button
                                    onClick={handleGenerateAlerts}
                                    variant="default"
                                    className="gap-2 w-full"
                                    disabled={parseResult.detections.length === 0}
                                >
                                    <Database className="h-4 w-4" />
                                    Generate Alerts ({parseResult.detections.length} threats detected)
                                </Button>
